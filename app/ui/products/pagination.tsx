...
function PaginationNumber({
  ...
  const className = clsx("flex h-10 w-10 items-center justify-center text-sm border border-gray-300", {
    "rounded-l-md": position === "first" || position === "single",
    "rounded-r-md": position === "last" || position === "single",
    "z-10 bg-black border-black text-white": isActive,
    "hover:bg-gray-100 bg-white": !isActive && position !== "middle",
    "text-gray-400 bg-white cursor-default": position === "middle",
  })
...
function PaginationArrow({
  ...
  const className = clsx("flex h-10 w-10 items-center justify-center rounded-md border border-gray-300 bg-white", {
  ...
  return isDisabled ? (
    <div className={className} aria-disabled="true" aria-label={label}>{icon}</div>
  ) : (
